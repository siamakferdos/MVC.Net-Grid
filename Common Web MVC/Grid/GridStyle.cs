...
namespace Ferdo.Common.Web.MVC.Grid
{
    public class GridStyle
    {
        ...
        /// 
        /// </summary>
        /// <param name="ferdoGridClass">Chose yor class from GridModel.GridClass Enum</param>
        /// <param name="cssStyle">Enter full css attribute and values in a string format. Something like this :
        /// "color:red; font-size:10px;"
        /// </param>
        public void SetGridStyle(GridClasses ferdoGridClass, string cssStyle)
        {
            if (GridStyles.ContainsKey(ferdoGridClass.ToString()))
            {
                GridStyles[ferdoGridClass.ToString()] = cssStyle;
            }
            else
            {
                GridStyles.Add(ferdoGridClass.ToString(), cssStyle);
            }
        }
        ...
        internal string GetGridStyle(GridClasses ferdoGridClass)
        {
            if (GridStyles.ContainsKey(ferdoGridClass.ToString()))
            {
                return " GridContainer" + _gridName + " ." + ferdoGridClass.ToString() + "{" + GridStyles[ferdoGridClass.ToString()] + "}";
            }
            return "";
        }
